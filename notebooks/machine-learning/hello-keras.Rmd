...
<b>Keras</b> is an open-source high-level neural networks API (originally written in Python) and capable of running on top of TensorFlow, CNTK, or Theano.

To get first hand experience with using Neural Networks for image classification, we'll use
<ol>
<li>Keras
<li>TensorFlow
<li>MNIST images of hand-written numbers
</ol>
...
We need to simplify this for our model, as is typical when testing a hypothesis. 
The question we have for this experiment is "What number is in this picture?"
...
model <- keras_model_sequential() %>% # initialize our model using a pre-baked model type (thanks R keras!)
  ...
  layer_dense(units = 10, activation = "softmax", name="prediction") # define the final layer transformation
```
